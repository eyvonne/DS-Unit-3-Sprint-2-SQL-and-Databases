...
Employee and Territory are connected via a join or corollary table. They have a one to many relationship, where each item on the employee table maps to multiple territories on the territory table. For example Robert King maps to ten territories.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

A document store like MongoDB is best for its flexibility and scalability. It does not have pre set types for each of its fields meaning that its much easier to scale up as new data comes in than something like postgres sql. It looses some function out though if data requires certain parameters to be input.

- What is "NewSQL", and what is it trying to achieve?

NewSQL is trying to create a new online database system with many of the advantages in terms of scalability of a noSQL document store (like MongoDB) and give it some of the ACID Guarantee. NewSQL favors consistency over availability while NoSQL relies more on 'eventual consistency' in favor of higher availability
